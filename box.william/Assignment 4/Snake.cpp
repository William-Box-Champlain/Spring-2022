...
Snake::Snake(Animation* snakeHeadAnimation, Animation* snakeBodyAnimation, Vector2D startingPosition, direction startingHeading)
{
	...
	mMovementDelayRemaining = 500.0f;
}
...
{
	for (size_t i = 0; i < segmentsToRemove; i++)
	{
		removeSegment();
	}
}
...
void Snake::updateSnake(double deltaTime) 
{
	...
	if (mMovementDelayRemaining <= 0)
	{
		...
		double headPositionScalar = mHead->getCurrentSprite()->getSpriteDimensions().getX();
